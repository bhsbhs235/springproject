...
if [ -z "$EXIST_BLUE" ]; then
    echo "blue up"
    /usr/local/bin/docker-compose up --build -p ${DOCKER_APP_NAME}-blue -f docker-compose.blue.yml up -d
    ...
    echo "green up"
    /usr/local/bin/docker-compose up --build -p ${DOCKER_APP_NAME}-green -f docker-compose.green.yml up -d
